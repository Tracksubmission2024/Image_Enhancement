% SSIM_vs_Alpha plot for input image from our dataset
clc;
clear;
close all;
...
alpha_values = 0:0.01:1.5;  % Alpha values from 0 to 1.5 with a step of 0.01
ssim_values = zeros(size(alpha_values));
for i = 1:length(alpha_values)
    ...
    ssim_values(i) = ssim(image_rgb, image_out);
end 
figure;
plot(alpha_values, ssim_values, 'bo-');
xlabel('Alpha');
ylabel('SSIM');
grid on;
title('SSIM vs Alpha');
