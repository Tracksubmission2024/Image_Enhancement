...
% Adjust the transfer functions
alpha = 0.75; % Adjustment parameter (increase or decrease as desired)
...
image_out_r=uint8(image_out_r);
image_out_g= uint8(image_out_g);
image_out_b=uint8(image_out_b);
figure(1)
image(image_rgb);
title('Input Image');
figure(2)
...
figure(3)
hold on
p(1)= plot(l,E_r,'-r','DisplayName','Input energy_R');
p(2)= plot(l,E_g,'-g','DisplayName','Input energy_G');
p(3)= plot(l,E_b,'-b','DisplayName','Input energy_B');
p(4)= plot(l,E_hat_r,'--r','DisplayName','Clipped energy_R');
p(5)= plot(l,E_hat_g,'--g','DisplayName','Clipped energy_G');
p(6)= plot(l,E_hat_b,'--b','DisplayName','Clipped energy_B');
xlabel('Intensity values');
ylabel('Energy values');
title('Input and Clipped Energy curves');
xline(L_low_r,'-.r'); xline(L_high_r,'-.r');
xline(L_low_g,'-.g'); xline(L_high_g,'-.g');
xline(L_low_b,'-.b'); xline(L_high_b,'-.b');
legend(p(1:6));
figure(4)
hold on
plot(l,T_r,'-r');
...
title('Modified Transfer functions');
legend('R','G','B')
figure(5)
hold on
plot(l,E_out_r,'-r');
